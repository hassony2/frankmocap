...
# default_argument_parser, default_setup, hooks, launch
...
class Third_View_Detector(BodyPoseEstimator):
    ...
    It combines a body pose estimator
    (https://github.com/jhugestar/lightweight-human-pose-estimation.pytorch.git)
        ...
        if num_bbox > 0:
            for idx, body_pose in enumerate(body_pose_list):
                ...
                if body_pose[7][0] > 0 and body_pose[6][0] > 0:
                    ...
                    dist_wrist_elbow = np.linalg.norm(body_pose[7] -
                                                      body_pose[6])
                    for i in range(num_bbox):
                        ...
                        if dist_bbox_ankle < dist_wrist_elbow * 1.5:
                            dist_left_arm[i] = np.linalg.norm(center -
                                                              body_pose[7])
                # right arm
                if body_pose[4][0] > 0 and body_pose[3][0] > 0:
                    # distance between elbow and ankle
                    dist_wrist_elbow = np.linalg.norm(body_pose[3] -
                                                      body_pose[4])
                    for i in range(num_bbox):
                        bbox = raw_hand_bboxes[i]
                        c_x = (bbox[0] + bbox[2]) / 2
                        ...
                        if dist_bbox_ankle < dist_wrist_elbow * 1.5:
                            dist_right_arm[i] = np.linalg.norm(center -
                                                               body_pose[4])
...
class Ego_Centric_Detector(BodyPoseEstimator):
    ...
    def __load_hand_detector(self):
        ...
        fasterRCNN = detector_resnet(classes,
                                     101,
                                     pretrained=False,
        ...
        if self.device == "gpu" or (self.device is None):
            fasterRCNN.cuda()
        fasterRCNN.eval()
    ...
    def __get_raw_obj_bbox(self, scores, pred_boxes, thresh_obj=0.5):
        j = 1
        obj_scores = scores[:, j]
        inds_obj = torch.nonzero(obj_scores > thresh_obj,
                                 as_tuple=False).view(-1)
        if inds_obj.numel() > 0:
            cls_boxes = pred_boxes[inds_obj][:, j * 4:(j + 1) * 4]
            cls_scores = obj_scores[inds_obj]
    ...
    # part of the code comes from https://github.com/ddshan/hand_object_detector/demo.py
    def __get_raw_hand_bbox(self, scores, pred_boxes, lr, thresh_hand=0.5):
        # Hand index
        j = 2
        hand_scores = scores[:, j]
        inds_hand = torch.nonzero(hand_scores > thresh_hand,
                                  as_tuple=False).view(-1)

        if inds_hand.numel() > 0:
            cls_boxes = pred_boxes[inds_hand][:, j * 4:(j + 1) * 4]
            cls_scores = hand_scores[inds_hand]

            _, order = torch.sort(cls_scores, 0, True)

            lr = lr[inds_hand][order]
            cls_boxes = cls_boxes[order]
            cls_scores = cls_scores[order]
    ...
    def detect_hand_bbox(self, img):
        ...
        scores, pred_boxes, lr = self.__get_raw_bbox_info(img)
        hand_bboxes, hand_types = self.__get_raw_hand_bbox(
            scores, pred_boxes, lr)
        ...
        if hand_bboxes is not None:
            ...
            right_bbox = hand_bboxes[hand_types == 1]
            if len(right_bbox) > 0:
                hand_bbox_list[0]['right_hand'] = right_bbox[0]
        if obj_bboxes is not None:
            if len(obj_bboxes):
                hand_bbox_list[0]['objects'] = obj_bboxes
...
class HandBboxDetector(object):
    def __init__(self, view_type, device):
        ...
        if view_type == "ego_centric":
            self.model = Ego_Centric_Detector(device=device)
                ...
                for hand_type in ["left_hand", "right_hand"]:
                    ...
                    if bbox is not None:
                        ...
                        hand_bbox[hand_type] = np.array(
                            [x0, y0, x1 - x0, y1 - y0])
                for hand_type in ["objects"]:
                    bbox = hand_bbox[hand_type]
                    if bbox is not None:
                        x0, y0, x1, y1 = bbox.transpose()
                        hand_bbox[hand_type] = np.stack(
                            [x0, y0, x1 - x0, y1 - y0], 1)
