...
def extract_hand_output(output,
                        hand_type,
                        hand_info,
    ...
    hand_verts_idx = torch.Tensor(
        hand_info[f'{hand_type}_hand_verts_idx']).long()
    if use_cuda:
        hand_verts_idx = hand_verts_idx.cuda()

    hand_verts = vertices[:, hand_verts_idx, :]
    hand_verts_shift = hand_verts - joints[:, hand_start_idx:hand_start_idx +
                                           1, :]

    # Hand joints
    if hand_type == 'left':
        hand_idxs = [20] + list(range(25, 40)) + list(range(
    ...
    hand_joints_shift = hand_joints - joints[:, hand_start_idx:hand_start_idx +
                                             1, :]

    output = dict(wrist_idx=wrist_idx,
...
class H3DWModel(object):
    ...
    def __init__(self, opt):
        ...
        assert (self.total_params_dim == self.cam_params_dim +
        ...
        self.input_img = self.Tensor(nb, opt.input_nc, self.inputSize,
                                     self.inputSize)
    ...
    def load_params(self):
        ...
        self.global_orient = torch.zeros((self.batch_size, 3),
                                         dtype=torch.float32).cuda()
        # self.global_orient[:, 0] = np.pi
        self.global_orient.requires_grad = False

        # load smplx-hand faces
        hand_info_file = osp.join(self.opt.model_root,
                                  self.opt.smplx_hand_info_file)

        self.hand_info = gnu.load_pkl(hand_info_file)
        self.right_hand_faces_holistic = self.hand_info[
            'right_hand_faces_holistic']
        self.right_hand_faces_local = self.hand_info['right_hand_faces_local']
        self.right_hand_verts_idx = np.array(
            self.hand_info['right_hand_verts_idx'], dtype=np.int32)
    ...
    def get_smplx_output(self, pose_params, shape_params=None):
        ...
        output = self.smplx(global_orient=self.global_orient,
                            body_pose=body_pose,
    ...
    def forward(self):
        ...
        self.pred_pose_params = self.final_params[:,
                                                  cam_dim:(cam_dim + pose_dim)]
